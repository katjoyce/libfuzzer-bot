...
echo =========== PULL target   && (cd $TARGET_NAME; git pull)
...
echo =========== FUZZING
./${TARGET_NAME}_asan_cov_fuzzer -max_len=$MAX_LEN $CORPUS  -artifact_prefix=CORPORA/ARTIFACTS/ -jobs=$J -workers=$J -max_total_time=$MAX_TOTAL_TIME -use_counters=$USE_COUNTERS > $L 2>&1
exit_code=$?
case $exit_code in
  0) prefix=pass
    ;;
