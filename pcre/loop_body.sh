...
echo =========== FUZZING
./pcre_asan_cov_fuzzer -max_len=$MAX_LEN $CORPUS  -artifact_prefix=CORPORA/ARTIFACTS/ -jobs=$J -workers=$J -max_total_time=$MAX_TOTAL_TIME -use_counters=0 > $L 2>&1
exit_code=$?
case $exit_code in
  0) prefix=pass
    ;;
...
echo =========== DUMP COVERAGE
rm -f *sancov
UBSAN_OPTIONS=coverage=1 ./pcre_func_fuzzer -max_len=$MAX_LEN $CORPUS -runs=0 > func_run.log 2>&1
dump_coverage pcre_func_fuzzer >> $L
